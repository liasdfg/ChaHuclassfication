...
### 1. ResNeXt-50 (32×4d)
* 针对紫砂壶细粒度分类任务中类间差异较小的问题，ResNeXt-50（32×4d）采用增加基数而非加深网络的方式，通过32条并行分支提取特征，各分支学习互补表示，在相近计算量下提升模型表达能力与判别性能。
* 各分支采用相同拓扑结构，并通过分组卷积实现统一计算，使不同路径在共享结构的同时关注不同特征子空间，从而提升局部细节建模能力。
* 多分支特征在输出端进行聚合融合，增强特征互补性，提升模型泛化能力与稳定性。


### 2. CBAM模块（Convolutional Block Attention Module）
* 考虑到茶壶主要靠轮廓比例、口盖线条、流把弧度区分，且紫砂泥料质感、窑变色泽、包浆光泽等细节信息在通道与空间维度上均需精准捕获，CBAM 模块通过串联通道注意力和空间注意力，能同时增强重要通道权重并聚焦关键空间区域，进一步提升判别能力。
...
1. **CBAM注意力机制**：融合通道注意力与空间注意力，从语义维度和空间维度同时强化重要特征，提高模型分类能力。
2. **ResNeXt分组卷积**：通过引入基数，在计算量可控的前提下扩展特征子空间表达能力，使模型能够学习更丰富的局部与全局特征组合。
3. **预激活Bottleneck**：采用预激活结构，改善梯度传播路径，缓解深层网络退化问题，提高训练收敛稳定性。
4. **多任务共享特征**：基于统一ResNeXt特征提取主干，通过多个独立分类头进行并行预测，实现参数共享，提升模型泛化能力与训练效率。
...
2. **训练控制策略**
   ...
   hybrid 混合模式：采用历史权重与当前权重平滑融合，使权重更新更平滑、训练更稳定，避免权重剧烈波动。
